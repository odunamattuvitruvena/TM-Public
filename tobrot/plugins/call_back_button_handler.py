...
async def button(bot, update: CallbackQuery):
    ...
    if "|" in cb_data:
        ...
        if cb_data.startswith("cancel"):
            if len(cb_data) > 1:
                i_m_s_e_g = await update.message.reply_text("𝗖𝗵𝗲𝗰𝗸𝗶𝗻𝗴...🔍", quote=True)
                ...
                try:
                    ...
                    if os.path.exists(file_name):
                        if os.path.isdir(file_name):
                            ...
                            os.remove(file_name)
                    await i_m_s_e_g.edit_text(f"𝗟𝗲𝗲𝗰𝗵 𝗖𝗮𝗻𝗰𝗲𝗹𝗹𝗲𝗱 𝗯𝘆 <a href='tg://user?id={update.from_user.id}'>{update.from_user.first_name}</a>")
                ...
                if len(g_del_list) != 0:
                    for f in g_del_list:
                        if os.path.isfile(f):
                            ...
                            shutil.rmtree(f)
                    await update.message.edit_text(f"𝗗𝗲𝗹𝗲𝘁𝗲𝗱 {len(g_del_list)} 𝗢𝗯𝗷𝗲𝗰𝘁𝘀 😬")
                else:
                    await update.message.edit_text("𝗡𝗼𝘁𝗵𝗶𝗻𝗴 𝘁𝗼 𝗰𝗹𝗲𝗮𝗿 🙄")
            else:
                await update.message.edit_text("𝗪𝗿𝗼𝗻𝗴 𝗨𝘀𝗲𝗿 🤭")
        elif cb_data == "fuckoff":
            await update.message.edit_text("𝗢𝗸𝗮𝘆! 𝗳𝗶𝗻𝗲 🤬")
